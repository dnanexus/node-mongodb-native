...
// Remove empty Mongo operators from `object`.
exports.sanitizeOperators = function(object) {
  var sanitizedObject = {};
...
// Validate that all fieldnames in `object` are compatible with Mongo 2.6.x.;
// that is, fieldnames cannot start with `'$'` or contain `'.'`. Can only be
// used to validate document insertions. Validating document updates is tricky
// because subdocument modifications can be specified using fieldnames that do
// contain `'.'`.
exports.validateFieldnames26 = function(object) {
  var re = /^\$|\./;

  var validate = function(object) {
    for (var fieldname in object) {
      if (object.hasOwnProperty(fieldname)) {
        if (re.exec(fieldname)) {
          throw new Error('Invalid fieldname "' + fieldname + '" for Mongo ' +
                          'Mongo 2.6.x');
        }
        // Recurse on subdocuments.
