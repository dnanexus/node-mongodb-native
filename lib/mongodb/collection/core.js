...
var insert = function insert (docs, options, callback) {
  ...
  if(!useLegacyOps && hasWriteCommands(connection) 
    ...
    && !(Array.isArray(docs) && docs.length > 0 && Buffer.isBuffer(docs[0]))) {
      ...
      var end = Date.now();
      if ((end - now) > 10000) {
        console.log('\ninsert took ' + (end - now) + ' ms to execute');
      }
      return this
  } 
...
var insertWithWriteCommands = function(self, docs, options, callback) {
  ...
  if(docs.length == 1) {
    ...
    var end = Date.now();
    if ((end - start) > 10000) {
      console.log('\ninsertWriteCommands took ' + (end - start) + ' ms to execute');
    }
...
var findAndModify = function findAndModify (query, sort, doc, options, callback) {
  ...
  if (doc && !options.remove) {
    queryObject.update = utils.sanitizeOperators(doc);
  }
  ...
  var end = Date.now();
  if ((end - now) > 10000) {
    console.log('\nfindAndModify took ' + (end - now) + ' ms to execute');
  }
}
