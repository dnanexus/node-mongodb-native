...
var find = function find () {
  ...
  if(len === 2 && !Array.isArray(fields)) {
    ...
    if(is_option) {
      ...
      // When presented with a call to collection.find that looks like:
      //
      //     find(query, arg, cb)
      //
      // the driver normally attempts to figure out whether argument 2 is a
      // field spec or an options hash. It does so by checking to see if arg
      // contains any keys that are recognized mongodb options (if so, the hash
      // is assumed to be options).
      //
      // This is really dangerous! Perhaps the most likely scenario where this
      // bites us is that a query is initially written with "fields" in a way
      // that doesn't include option names, and then a later-- and very
      // innocent-looking-- change introduces another field that coincides with
      // an option name, changing the entire meaning of the argument.
      //
      // Here we detect and prevent such ambiguous queries by failing any
      // 3-argument find(query, arg, cb) calls where "arg" does NOT contain
      // option names (i.e. "arg" is being interpreted as a field spec, but
      // only until someone adds an unfortunately named field...). You must
      // write such queries with 4 arguments instead to resolve the ambiguity:
      //
      //     find(query, field, options, cb)
      var err = new Error('Ambiguous 3-arg call find(query, fields, cb) to collection.find! Rewrite your query as find(query, fields, options, cb)');
  ...
  var end = Date.now();
  if ((end - start) > 10000) {
    console.log('\nfind took ' + (end - start) + ' ms to execute');
  }
...
var findOne = function findOne () {
  ...
  var end = Date.now();
  if ((end - start) > 10000) {
    console.log('\nfindOne took ' + (end - start) + ' ms to execute');
  }
};
