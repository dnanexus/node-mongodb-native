    ...
    if(Array.isArray(command)) {
      for(var i = 0; i < command.length; i++) {
        try {
          ...
          if (process.domain && process.domain.data !== undefined) {
            process.domain.data.bytesSent += binaryCommand.length;
            this.domain_data = process.domain.data;
          } else {
            this.domain_data = undefined;
          }
        } catch(err) {
      ...
      if(self.bytesRead > 0 && self.sizeOfMessage > 0) {
        ...
        if(self.stubBuffer != null && self.stubBuffer.length > 0) {
          ...
          if(data.length > 4) {
            ...
            if (self.domain_data) {
              self.domain_data.bytesRead += sizeOfMessage;
            }
